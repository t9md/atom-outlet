...
function getCenterPaneForOutlet ({useAdjacentPane, splitDirection}) {
  ...
  const pane = splitPane(basePane, splitDirection)
  activePane.activate()
  return pane
}
...
function getPaneForLocation (editor, location = null) {
  const [dockOrCenter, direction] = editor.element.getAttribute('outlet-default-location').split('-')

  if (!location) {
    location = dockOrCenter === 'center' ? 'center' : direction
  }
